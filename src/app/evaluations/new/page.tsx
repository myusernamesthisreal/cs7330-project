                            ...
                            <div className="grid grid-cols-3 gap-4">
                                {objectives.map((objective, index) => (
                                    <div key={index} className="border p-4 rounded-lg flex flex-col">
                                        ...
                                        {<button disabled={objective.evaluation === null} onClick={(e) => router.push(`/evaluations/copy?objectiveId=${objective.evaluation.learningObjectiveId}&sectionNumber=${objective.evaluation.sectionNumber}&degreeName=${objective.evaluation.degreeName}&degreeLevel=${objective.evaluation.degreeLevel}`)} className={objective.evaluation !== null ? "bg-blue-500 text-white rounded-lg p-2 mt-4 w-full" : "bg-blue-500 text-white rounded-lg p-2 mt-4 w-full opacity-50 cursor-not-allowed"}>Duplicate</button>}
                                    </div>
                                ))}
                            </div>
